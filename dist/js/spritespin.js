...
(function($) {
  ...
  // example:
  //  sourceArray('http://example.com/image_{frame}.jpg, 1, 3, 2)
  //  -> [ 'http://example.com/image_01.jpg', 'http://example.com/image_02.jpg' ]
  Spin.sourceArray = function(path, start, end, digits){
    var i, result = [];
    for (i = start; i <= end; i+=1){
      result.push(path.replace("{frame}", padNumber(i, digits, 0)));
    }
    return result;
  };
  ...
  Spin.measureSource = function(data){
    if (data.images.length === 1){
      ...
      if (data.framesX){
        data.frameWidth = data.sourceWidth / data.framesX;
        data.frameHeight = data.sourceHeight / data.sourceWidth * data.frameWidth;
      }
    } else {
      data.frameWidth = data.images[0].width;
      data.frameHeight = data.images[0].height;
      data.sourceWidth = data.frameWidth;
      data.sourceHeight = data.frameHeight;
      data.frames = data.images.length;
    }
  };
  ...
  Spin.updateFrame = function(data, frame){
  ...
  Spin.init = function(options){
    ...
    if (data.behavior || data.module){
      ...
      data.behavior = data.module = undefined;
    }
  ...
  // Receives an object where keys are method names and values are methods
